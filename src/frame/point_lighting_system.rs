...
layout(push_constant) uniform PushConstants {
    ...
    vec4 position;
    // The `radius` parameter of the `draw` method.
    float radius;
} push_constants;
...
void main() {
    ...
    vec3 light_direction = normalize(world.xyz - push_constants.position.xyz);
    ...
    float light_distance = length(world.xyz - push_constants.position.xyz);
    float atten = (light_distance / push_constants.radius) + 1;
    // Further decrease light_percent based on the distance with the light position.
    light_percent *= 1.0 / (atten * atten);
